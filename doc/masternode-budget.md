...
Eve now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month. 
...
mnbudget prepare proposal-name url payment_count block_start eve_address monthly_payment_eve [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ EVE, _12_ times over the course of a year totaling _24000_ EVE. 
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` EVE. 

The following new RPC commands are supported:
- mnbudget "command"... ( "passphrase" )
 - prepare            - Prepare proposal for network by signing and creating tx
 - submit             - Submit proposal for network
 - vote-many          - Vote on a Eve initiative
 - vote-alias         - Vote on a Eve initiative
 - vote               - Vote on a Eve initiative/budget
